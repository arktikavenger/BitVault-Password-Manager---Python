...
class InstallWizard(QtWidgets.QWizard, object):
    def __init__(self):
        ...
        if os.path.exists(os.path.dirname(__file__) + "/" + self.installer_script):
            self.installer_script = os.path.dirname(
                __file__) + "/" + self.installer_script

        # TODO: Make sure it is actually executable

        self.should_show_last_page = False
        self.error_message_nice = tr(
            "An unknown error occurredi...Ghost in the machine...ERR208")
    ...
    def installerLogButtonClicked(self):
        ...
        command = 'launch'
        args = ['QTerminal', '-e', 'tail', '-f',
                self.logfile, self.errorslogfile]
        print(args)
        try:
...
supported_locales = [f for f in os.listdir(
    localepath) if os.path.isdir(os.path.join(localepath, f))]
...
supported_layouts = [f for f in os.listdir(
    localepath) if os.path.isfile(os.path.join(localepath, f))]
...
class LanguagePage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        if shutil.which(wizard.installer_script) is None:
            wizard.showErrorPage(
                tr("The installer script was not found. Please make sure that you are running the installer on a supported system."))
        ...
        json_file = QtCore.QFile(os.path.dirname(__file__) + '/languages.json')
        if json_file.open(QtCore.QIODevice.ReadOnly):
            document = QtCore.QJsonDocument().fromJson(json_file.readAll())
            ...
            if language in supported_languages:
                try:
                    self.listwidget.addItem(
                        self.languages[language]["nativeName"].toString())
                except:  # FIXME: do not use bare 'except'
        ...
        try:
            ...
            for output_line in output_lines:
                if "layout:" in str(output_line):
                    system_keyboard_layout = str(output_line.split(":")[
                                                 1].trimmed().split(",")[0])
                    if system_keyboard_layout != "EN" and system_keyboard_layout in supported_layouts:
                        print(
                            "System keyboard layout is already set to %s which is a supported keyboard layout, hence using this" % system_keyboard_layout)
            ...
            if "LANG" in os.environ:
                ...
                if "_" in lang_env and (lang_env != "en_US" or system_keyboard_layout != "EN"):
                    if lang_env in supported_locales:
                        print("System is already set to LANG=%s which is a supported locale, hence using this without further questions" % (
                            lang_env))
                        if wizard.selected_language is None:
                            # If we already got one from system_keyboard_layout, use that
                            wizard.selected_language = lang_env.split("_")[0]
    ...
    def clicked1(self):
                    ...
                    if country.lower() in supported_layouts:
                        if language_spoken == wizard.selected_language:
                            ...
                            self.listwidget2.addItem(
                                countries[country]["name"].toString())
                            break
        if len(wizard.candidate_countries) == 1:
            ...
            print(
                "Country selection is needed since this language is spoken in more than one supported locales")
    ...
    def isComplete(self):
        if wizard.selected_language is not None and wizard.selected_country is not None:
            print("Determined locale to be %s_%s" %
                  (wizard.selected_language, wizard.selected_country))
            return True
        else:
            return False
...
class CountryPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.setSubTitle(
            tr("To set up the locale and keyboard layout, select a country."))
    ...
    def initializePage(self):
        ...
        QtCore.QObject.connect(self.listwidget, QtCore.SIGNAL(
            "itemClicked(QListWidgetItem *)"), self.clicked)
...
class IntroPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.setSubTitle(
            tr("To set up the installation of Morales Research Unix OS X, click 'Continue'."))

        logo_pixmap = QtGui.QPixmap(os.path.dirname(
            __file__) + '/FREEBSD_Logo_Vert_Pos_RGB.png').scaledToHeight(200, QtCore.Qt.SmoothTransformation)
        ...
        intro_vLayout.addWidget(center_widget, True)
        ...
        # True = add stretch vertically
        intro_vLayout.addWidget(intro_label, True)
...
class LicensePage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.setSubTitle(
            tr('To continue installing the software, you must agree to the terms of the software license agreement.'))
        ...
        additional_licenses_label.setText(
            tr("Additional components may be distributed under different licenses as stated in the respective documentation."))
...
class DiskPage(QtWidgets.QWizardPage, object):
    ...
    def getMiBRequiredOnDisk(self):
        ...
        proc = QtCore.QProcess()
        command = 'sudo'
        # -E to pass environment variables into the command ran with sudo
        args = ["-n", "-E", wizard.installer_script]
    ...
    def initializePage(self):
        ...
        if wizard.required_mib_on_disk < 5:
            self.timer.stop()
            wizard.showErrorPage(
                tr("The installer script did not report the required disk space. Are you running the installer on a supported system? Can you run the installer script with sudo without needing a password?"))
...
class RootPwPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.registerField('rootpw*', rootpw_lineedit)
...
class UserPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.setSubTitle(
            tr('Please enter details to create an account for the main user on this computer.'))
        ...
        self.autologin_checkbox.setText(
            "To be implemented: Log this user automatically into the desktop (autologin)")
        ...
        self.sshd_checkbox.setText(
            tr("Enable users to log in over the network (ssh)"))
        ...
        self.timezone_checkbox.setText(
            tr("Set time zone based on current location"))
        # self.sshd_checkbox.setWordWrap(True) # Does not work, https://bugreports.qt.io/browse/QTBUG-5370
        ...
        self.user_label_comment.setText(
            tr("The passwords do not match"))  # TODO: make red
    ...
    def initializePage(self):
        ...
        if internetCheckConnected() is False:
            ...
            print(
                "Not offering to set time zone based on currrent location because not connected to the internet")

    def populateUsername(self):
        if " " in self.field('fullname'):
            generated_username = self.field('fullname').split(" ")[0].lower(
            )[0] + self.field('fullname').split(" ")[len(self.field('fullname').split(" ")) - 1].lower()
        else:
        ...
        if self.field('enable_ssh') is True:
            ...
            if not "." in self.field('computername'):
                self.hostname_label.setText(
                    "ssh %s@%s.local." % (self.field('username'), self.field('computername')))
            else:
                self.hostname_label.setText(
                    "ssh %s@%s" % (self.field('username'), self.field('computername')))
        else:
            self.hostname_label.hide()
    ...
    def computerName(self):
        ...
        # We never want to use this string for anything
        undefined = "To be filled by O.E.M."
...
class InstallationPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        self.setSubTitle(
            tr('Morales Research Unix is being installed to your computer.'))
    ...
    def initializePage(self):
        ...
        if wizard.selected_disk_device is None or os.path.exists(dev_file) is False:
            wizard.showErrorPage(
                tr("The selected disk device %s is not found.") % dev_file)
            return  # Stop doing anything here

        if wizard.user_agreed_to_erase is False:
            wizard.showErrorPage(
                tr("Did not get permission to erase the target device."))
            return  # Stop doing anything here
        ...
        if wizard.selected_language is not None and wizard.selected_country is not None:
            if wizard.selected_language + "_" + wizard.selected_country in supported_locales_utf8:
                computed_locale_utf8 = wizard.selected_language + \
                    "_" + wizard.selected_country + ".UTF-8"
    ...
    def onProcessFinished(self):
        ...
        if (exit_code != 0):
            wizard.showErrorPage(
                tr("The installation did not succeed. Please see the Installer Log for more information."))
            return  # Stop doing anything here
        else:
            wizard.next()
    ...
    def periodicallyCheckProgress(self):
        ...
        self.checkProgress()
        # Used to periodically check the fill level of the target disk
    ...
    def checkProgress(self):
        ...
        print("%i of %i MiB on target disk" %
              (self.mib_used_on_target_disk, wizard.required_mib_on_disk))
...
class SuccessPage(QtWidgets.QWizardPage, object):
    ...
    def initializePage(self):
        ...
        self.setSubTitle(
            tr('The installation of Morales Research Unix OS X succeeded.'))

        logo_pixmap = QtGui.QPixmap(os.path.dirname(
            __file__) + '/check.png').scaledToHeight(256, QtCore.Qt.SmoothTransformation)
        logo_label = QtWidgets.QLabel()
        logo_label.setPixmap(logo_pixmap)
        ...
        label.setText(
            tr("MR Unix OS X has been installed on your computer, click 'Restart' to begin using it."))
        layout.addWidget(label)

        self.setButtonText(wizard.NextButton, tr("Restart"))
        wizard.button(QtWidgets.QWizard.NextButton).clicked.connect(
...
class ErrorPage(QtWidgets.QWizardPage, object):
    def __init__(self):
        ...
        logo_pixmap = QtGui.QPixmap(os.path.dirname(
            __file__) + '/cross.png').scaledToHeight(256, QtCore.Qt.SmoothTransformation)
        logo_label = QtWidgets.QLabel()
        logo_label.setPixmap(logo_pixmap)
    ...
    def initializePage(self):
        ...
        wizard.setButtonLayout([QtWidgets.QWizard.CustomButton1,
